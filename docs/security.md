...
When `HermesKit.useNIOSSHTransport` is enabled (mandatory on iOS, opt-in on macOS), HermesKit uses a pure-Swift SSH client (`swift-nio-ssh`) instead of the system `ssh` binary for the ACP transport. Dashboard mode still uses system `ssh -L` on macOS; on iOS, dashboard HTTP runs through the NIO-SSH `direct-tcpip` tunnel owned by the window. Consequence: on macOS with the flag enabled, host-key trust can be evaluated by both system-ssh and HermesKit's ``HostKeyStore`` depending on the surface. They can disagree on first connect, so UI must surface the exact transport error instead of silently retrying through a different path.
...
- **Background sessions.** Long-lived SSH sessions die when an iOS app backgrounds. The iOS app target needs an explicit reconnect strategy for ACP and dashboard transports.
