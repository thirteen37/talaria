...
5. Interrupt a running session. While a turn is streaming, also confirm the
   ...
   one-time tip, then a terse "Queued.") — Talaria holds the message locally and
   submits it as a new turn when the current turn ends *cleanly*; queue two and
   confirm they drain FIFO (one per clean turn), and that interrupting a turn
   halts the drain (held items resume after the next clean turn). `/steer …`
   redirects the running turn; `/bg <prompt>` starts a concurrent background task
   (a "🌘 N running" indicator appears, the foreground turn is unaffected, and a
   "🌖 Background task finished" line lands when it completes). The slash menu
   offers the pending-input commands (`queue`, `q`, `steer`, `plan`, `goal`,
   `retry`, `undo`) plus the background commands (`bg`, `background`, `btw`), and
   Send is disabled for a non-pending slash like `/help`. The live turn keeps
   streaming throughout (no new user bubble). Confirm interrupt works via the
   Cancel button, `⌘.` (one chord), and a double-`Esc` (first arms with a "Press
   Esc again to interrupt" hint, second cancels) — and that a single `Esc` while
   the slash menu is open just dismisses the menu.
...
18. **Dashboard lifecycle**: open two windows for the same profile and confirm they share one `hermes dashboard` process. Close both and verify the child exits.
    ...
    - **iOS/iPad WS-only death (HTTP still healthy)**: with a live chat open, restart just the gateway/`/api/ws` socket while leaving the dashboard HTTP reachable (e.g. bounce the gateway, or reset the WS `direct-tcpip` channel) — the `/api/status` probe still passes. Background and foreground the app. Confirm the open chats silently re-resume over the still-good tunnel (no full "Reconnecting…" teardown) and accept a new prompt, rather than staying dead until a manual Reconnect.
    - **iOS/iPad cold-relaunch restoration** (iPhone **and** iPad): the app should come back the way it was left after iOS terminates the suspended app.
        - **Full restore**: open a chat (with history), and — on iPhone — drill into a nested Browse page (Browse → a manage surface → deeper) and/or open a sheet; on iPad, navigate to a Browse page or open Settings. Hard-kill the app from the app switcher, then relaunch. Confirm the chat re-resumes with its history, the Browse depth + sheet are restored, the selected chat is reselected, and no stale state from a *different* profile leaks in.
        - **Server-deleted session**: server-side-delete one of the open sessions, then cold-relaunch. Confirm that tab degrades gracefully (it isn't re-opened, no error spam) while the rest restore, and the selection falls back to a tab that did re-open.
        - **User intent wins**: cold-launch, then *immediately* tap a different session (via All Sessions) or a Browse row before the dashboard finishes connecting. Confirm the disk restore is abandoned — your live choice is not clobbered when the dashboard comes up.
        - **Offline relaunch preserves the snapshot**: with a saved session (open a chat, then hard-kill), relaunch while the server is **unreachable** (dashboard never connects), then background the app again without touching anything. Bring the server back and cold-relaunch once more. Confirm the original chat **still restores** — the offline launch must not have overwritten the saved snapshot with empty state.
        - **Warm reconnect is unaffected**: a plain in-memory background→foreground round-trip (app not killed) must still re-resume the *existing* open tabs via the connection recovery path above — it must **not** trigger a disk-backed restore (no duplicate tabs, no reset navigation).
        - **macOS unaffected**: confirm none of this changes desktop behavior — macOS windows aren't suspended/terminated, so navigation and open chats persist in-memory as before.
