...
public enum HermesUpdates {
    ...
    /// Pick the first line that contains "update available" (case-
    /// insensitive). Hermes' source-install notice is multi-line with a
    /// "Run 'hermes update' to install." footer below; the headline comes
    /// first, so the first match is the line we want.
